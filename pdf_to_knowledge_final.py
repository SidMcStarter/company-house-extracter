...
def convert_pdf_file_to_json(client, pdf_file_path, to_directory):
    """
    Convert a single PDF file to JSON using Azure OCR.

    Args:
        client (ComputerVisionClient): Azure Computer Vision client.
        pdf_file_path (str): Path to the PDF file.
        to_directory (str): Directory to save the JSON file.
    ...
    total_pages = len(doc)
    print(f"[INFO] PDF {os.path.basename(pdf_file_path)} has {total_pages} pages.")
    
    json_path = os.path.join(to_directory, f"{os.path.basename(pdf_file_path)[:-4]}.json")
    os.makedirs(to_directory, exist_ok=True)
    print("CHECKING JSON PATH", json_path)            
    if os.path.exists(json_path):
        print(f"[CACHE] Found OCR JSON at {json_path}. Loading from cache...")
